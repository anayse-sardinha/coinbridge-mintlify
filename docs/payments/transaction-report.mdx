...
<Steps>
  <Step title="Receive the report">
    Check for the incoming `TransactionReportDTO` at your endpoint.
  </Step>
  <Step title="Verify the result">
    Confirm the `approved` status within the `transaction` object.
  </Step>
  <Step title="Process the balance">
  ...
  <Step title="Update History">
    Reflect the transaction status, merchant name, and amount in the user’s transaction history.
  </Step>
</Steps>
